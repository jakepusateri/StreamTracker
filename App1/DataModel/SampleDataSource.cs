...
namespace App1.Data
{
    ...
    public sealed class SampleDataSource
    {
        ...
        public static async Task<JsonArray> GetArrayAsync(string uri)
        {
            var httpClient = new HttpClient();
            //httpClient.DefaultRequestHeaders.Add("user-agent", "");
            var content = await httpClient.GetStringAsync(uri);
            return await Task.Run(() => JsonArray.Parse(content));
        }
        ...
        private static async Task<SampleDataGroup> makeJustinGroupByName(string name){
            string url = "http://api.justin.tv/api/channel/show/" + name + ".json";

            JsonObject json = await GetAsync(url);

            int uid = (int)json.GetNamedNumber("id");
            string title = name;
            string subtitle = json.GetNamedString("channel_url");
            //string imagePath = json.GetNamedString("screen_cap_url_huge");
            string imagePath = json.GetNamedString("image_url_huge");
            //string description = json.GetNamedString("status");


            return new SampleDataGroup(uid.ToString(), title, subtitle, imagePath, "");
        }
        
        public static async Task clearGroups(){
            _sampleDataSource._allGroups.Clear();
        }
            //http://api.justin.tv/api/channel/archives/officecam.json?limit=50
        }
    }
